...
class DoubleConv(nn.Moudle):
    def __init__(self, in_channals, out_channals, mid_channels=None):
        ...
        if not mid_channels:
            mid_channels = out_channals
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channals, mid_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, out_channals, kernel_size=3, padding=1),
